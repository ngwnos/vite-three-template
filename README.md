...
`bun run test:e2e:webgpu` launches Chromium with WebGPU flags, starts `dev:local`, opens the app, and fails on page errors, console errors, or WebGPU/WGSL warnings. It does not go through the Idea Orca wrapper. It prefers `/snap/bin/chromium` if present. Otherwise set `PLAYWRIGHT_CHROMIUM_PATH` or install Playwright's Chromium browser.
